  ...
  return (
    <div className="flex-1 p-6 max-w-7xl mx-auto w-full space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-neutral-800">
        <div>
          <h2 className="text-xl font-bold text-neutral-100">{t.team.title}</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {ROLE_PERMISSIONS.map((rp) => (
          <div key={rp.role} className="p-4 rounded-xl bg-neutral-900 border border-neutral-800 space-y-2">
            ...
            <p className="text-xs text-neutral-400 leading-relaxed">{rp.desc}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-xl overflow-hidden shadow-lg">
        <div className="px-5 py-4 border-b border-neutral-800 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-neutral-100">{t.team.membersCount} ({allUsers.length})</h3>
          <span className="text-2xs text-neutral-500">Tenant: {currentWorkspace.name}</span>
        </div>
                  ...
                  <tr key={user.id} className="hover:bg-neutral-850/50 transition-colors">
                    <td className="px-5 py-3.5 flex items-center gap-3">
                      <div className="relative">
                        ...
                        />
                        {isCurrent && (
                          <span className="absolute -bottom-1 -right-1 w-3 h-3 bg-indigo-500 rounded-full border-2 border-neutral-900" />
                        )}
                      </div>
                      <div>
                    ...
                    <td className="px-4 py-3.5 text-neutral-400 font-mono">{user.email}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs animate-in fade-in">
          <div className="w-full max-w-md bg-neutral-900 border border-neutral-800 rounded-xl shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-800">
              <h3 className="text-base font-semibold text-neutral-100">{t.team.inviteModalTitle}</h3>
              <button
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2 text-xs bg-neutral-950 border border-neutral-800 rounded-lg text-neutral-100 placeholder-neutral-500 focus:outline-hidden focus:border-indigo-500"
                  />
                </div>
              </div>
