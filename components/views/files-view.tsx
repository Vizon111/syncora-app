  ...
  return (
    <div className="flex-1 p-6 max-w-7xl mx-auto w-full space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-neutral-800">
        <div>
          <h2 className="text-xl font-bold text-neutral-100">{t.files.title}</h2>
      ...
      >
        <div className="flex flex-col items-center justify-center space-y-2">
          ...
          <h4 className="text-sm font-semibold text-neutral-200">{t.files.dragDropTitle}</h4>
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="relative w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-1.5 text-xs bg-neutral-900 border border-neutral-800 rounded-lg text-neutral-200 placeholder-neutral-500 focus:outline-hidden"
          />
        </div>
      ...
      <div className="bg-neutral-900 border border-neutral-800 rounded-xl overflow-hidden shadow-lg">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-neutral-950/80 border-b border-neutral-800 text-neutral-400 uppercase tracking-wider text-2xs">
              <tr>
            ...
            <tbody className="divide-y divide-neutral-800/60 text-neutral-300">
              {filteredFiles.map((file) => (
                <tr key={file.id} className="hover:bg-neutral-850/50 transition-colors">
                  <td className="px-5 py-3.5 flex items-center gap-2.5 font-medium text-neutral-100">
                    <FileText className="w-4 h-4 text-indigo-400 shrink-0" />
                    <span className="truncate max-w-xs">{file.name}</span>
                  </td>
                  <td className="px-4 py-3.5 text-neutral-400 font-mono text-2xs">{file.type}</td>
                  ...
                  <td className="px-4 py-3.5 text-neutral-400">{file.uploadedBy?.name || t.tasks.teamDefault}</td>
          ...
          <div className="w-full max-w-2xl bg-neutral-900 border border-neutral-800 rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-800">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-neutral-100">{selectedFile.name}</h3>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
              <span className="text-2xs font-semibold text-neutral-400 uppercase tracking-wider block mb-2">
                {t.files.previewTitle}
              </span>
              <pre className="p-4 rounded-lg bg-neutral-950 border border-neutral-800 text-neutral-300 font-mono text-xs whitespace-pre-wrap leading-relaxed">
