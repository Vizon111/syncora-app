...
export function ExportStandupModal({ isOpen, onClose }: ExportStandupModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 dark:bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 rounded-3xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="p-5 border-b border-slate-200 dark:border-neutral-800 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-800 dark:text-neutral-100">{t.exportStandup.title}</h2>
              <p className="text-xs text-slate-500 dark:text-neutral-400 mt-0.5">{t.exportStandup.subtitle}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Tab switcher */}
        <div className="flex items-center gap-2 p-3 border-b border-slate-200/80 dark:border-neutral-800/80 bg-slate-50/40 dark:bg-neutral-950/40 text-xs">
          <button
        ...
        <div className="flex-1 p-5 overflow-y-auto custom-scrollbar">
          {activeTab === 'standup' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-600 dark:text-neutral-300 uppercase tracking-wider">
              ...
              <div className="p-4 rounded-2xl bg-slate-50 dark:bg-neutral-950 border border-slate-200 dark:border-neutral-800 text-slate-700 dark:text-neutral-200 font-mono text-2xs whitespace-pre-wrap leading-relaxed max-h-72 overflow-y-auto custom-scrollbar">
                {standupMarkdown}
              </div>
            </div>
          )}
          ...
          {activeTab === 'csv' && (
            <div className="space-y-4 text-center py-6">
              ...
              <h3 className="text-sm font-bold text-slate-700 dark:text-neutral-200">
              ...
              <div className="text-2xs font-mono text-slate-500 dark:text-neutral-500">
        ...
        {/* Footer */}
        <div className="p-4 border-t border-slate-200 dark:border-neutral-800 bg-white/60 dark:bg-neutral-900/60 flex justify-end">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-xl text-xs font-semibold text-slate-600 dark:text-neutral-300 hover:text-slate-800 dark:hover:text-neutral-100 hover:bg-slate-100 dark:hover:bg-neutral-800 transition-colors"
          >
            {t.common.close}
          </button>
        </div>
