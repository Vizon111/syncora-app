...
export function AutomationsModal({ isOpen, onClose }: AutomationsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 dark:bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden flex flex-col">
        {/* Header */}
        <div className="p-5 border-b border-slate-200 dark:border-neutral-800 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-800 dark:text-neutral-100">{t.automations.title}</h2>
              <p className="text-xs text-slate-500 dark:text-neutral-400 mt-0.5">{t.automations.subtitle}</p>
            </div>
          </div>
          <button
            onClick={onClose}
            ...
            return (
              <div
                ...
                className={`p-4 rounded-2xl border transition-all ${
                  rule.enabled
                    ? 'bg-slate-50/60 dark:bg-neutral-950/60 border-slate-200 dark:border-neutral-800'
                    : 'bg-slate-50/50 dark:bg-neutral-950/20 border-slate-250 dark:border-neutral-850 opacity-60'
                }`}
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h4 className="text-xs font-bold text-slate-700 dark:text-neutral-200">{title}</h4>
                      <p className="text-3xs text-slate-500 dark:text-neutral-400 mt-0.5 leading-relaxed">{desc}</p>
                      {rule.lastTriggered && (
                        <span className="inline-block mt-2 text-3xs text-slate-500 dark:text-neutral-500 font-mono">
                  ...
                  <button
                    ...
                    className="p-1 text-slate-600 dark:text-neutral-300 hover:text-white transition-colors shrink-0"
                    title={rule.enabled ? t.automations.active : t.automations.inactive}
                  >
                    {rule.enabled ? (
                      <ToggleRight className="w-7 h-7 text-indigo-500" />
                    ) : (
                      <ToggleLeft className="w-7 h-7 text-slate-600 dark:text-neutral-300" />
                    )}
                  </button>
                </div>
              </div>
        ...
        <div className="p-4 border-t border-slate-200 dark:border-neutral-800 bg-white/60 dark:bg-neutral-900/60 flex items-center justify-between">
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold bg-slate-100 dark:bg-neutral-800 hover:bg-slate-200 dark:hover:bg-neutral-700 text-slate-700 dark:text-neutral-200 transition-colors disabled:opacity-50"
          >
            <RotateCw
