      ...
      <div className="w-full max-w-lg bg-neutral-900 border border-neutral-800 rounded-xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-neutral-800 bg-neutral-900/50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-semibold text-neutral-100">{proposal.title}</h3>
              <p className="text-xs text-neutral-400">{t.ai.modalTitle}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <span className="text-xs font-semibold text-neutral-400 uppercase tracking-wider">{t.ai.actionProposal}</span>
            <p className="text-sm text-neutral-200 mt-1">{proposal.description}</p>
          </div>

          {/* Structured Payload Preview */}
          <div className="bg-neutral-950 p-3.5 rounded-lg border border-neutral-800">
            <span className="text-xs font-mono text-neutral-400 block mb-2">{t.ai.payloadPreview}</span>
            <div className="space-y-1.5 text-xs text-neutral-300 font-mono">
              <div className="flex">
                <span className="text-neutral-500 w-24 shrink-0">{t.common.actions}:</span>
                <span className="text-indigo-400">{proposal.type}</span>
              </div>
              <div className="flex">
                <span className="text-neutral-500 w-24 shrink-0">{t.tasks.taskTitle}:</span>
                <span className="text-neutral-200">{proposal.payload?.title}</span>
              </div>
              {proposal.payload?.status && (
                <div className="flex">
                  <span className="text-neutral-500 w-24 shrink-0">{t.common.status}:</span>
                  <span className="text-emerald-400">{proposal.payload.status.toUpperCase()}</span>
                </div>
              )}
              {proposal.payload?.priority && (
                <div className="flex">
                  <span className="text-neutral-500 w-24 shrink-0">{t.tasks.priorityLabel}:</span>
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-3 px-5 py-3.5 bg-neutral-950 border-t border-neutral-800">
          <button
            onClick={onClose}
